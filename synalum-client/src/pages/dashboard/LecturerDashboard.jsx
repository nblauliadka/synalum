...
export default function LecturerDashboard() {
  ...
  const loadMyOpportunities = useCallback(async () => {
    // Ambil semua peluang dari backend, lalu filter yang dibuat oleh user login (dosen)
    // Catatan: untuk MVP, kita filter di frontend karena endpoint "my opportunities" belum dibuat.
  ...
  async function handleCreateOpportunity(e) {
    ...
    try {
      // Backend akan memberi badge RESMI_USK otomatis jika role user = dosen
      await api.post('/api/opportunities/create', {
        title: title.trim(),
        description: description.trim(),
      })

      setMessage('Peluang berhasil diposting sebagai RESMI USK.')
      setTitle('')
      setDescription('')
      ...
      const msg =
        error?.response?.data?.message ||
        'Gagal posting peluang. Pastikan Anda login sebagai dosen dan server aktif.'
      setMessage(msg)
    } finally {
    ...
    <main className="min-h-screen bg-background text-text">
      <div className="mx-auto max-w-6xl px-4 py-10 md:px-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-lg font-semibold text-primary">
                Posting Peluang (Dosen)
              </div>
              <p className="mt-1 text-sm text-black/60">
                Postingan dosen akan diberi badge <span className="font-semibold">RESMI USK</span> oleh backend.
              </p>
            </div>
          ...
          <form className="mt-5 space-y-4" onSubmit={handleCreateOpportunity}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Contoh: Asisten Riset - Lab USK"
        ...
        <div className="mt-8 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5">
          ...
          <div className="mt-4 space-y-3">
            {[
              { title: 'Asisten Riset AI - Lab USK', badge: 'RESMI USK' },
              { title: 'Magang Kampus Merdeka - Partner', badge: 'EKSTERNAL' },
            ].map((item) => (
              <div
                key={item.title}
